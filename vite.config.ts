...
/**
 * 处理 index.html 中的 Umami Analytics 脚本：
 * - 当 VITE_ANALYTICS_ENDPOINT 和 VITE_ANALYTICS_WEBSITE_ID 均配置时，替换占位符
 * - 否则移除脚本，避免 %VAR% 未替换导致 URI 解码错误
 * - Umami Cloud 使用 script.js，自托管通常使用 umami.js
 */
function vitePluginAnalytics(): Plugin {
  return {
    ...
    transformIndexHtml(html) {
      ...
      const scriptPath = process.env.VITE_ANALYTICS_SCRIPT_PATH?.trim() || "script.js";
      if (endpoint && websiteId) {
        return html
          .replace(/%VITE_ANALYTICS_ENDPOINT%/g, endpoint)
          .replace(/%VITE_ANALYTICS_SCRIPT_PATH%/g, scriptPath)
          .replace(/%VITE_ANALYTICS_WEBSITE_ID%/g, websiteId);
      }
      return html.replace(
        /<script[\s\S]*?%VITE_ANALYTICS_ENDPOINT%[\s\S]*?<\/script>\s*/g,
        ""
      );
    },
  };
